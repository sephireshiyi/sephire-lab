...
> 本文件是项目级 AGENTS.md，会自动加载到每个 Codex 会话上下文。
> 内容仅包含**跨 role 的项目契约**——任何角色都必须遵守。
> Role 专属工作流见 `doc/ai/<role>/AGENTS.md`，按需 lazy 加载。
...
项目有三个 Codex role，每个 role 独立 session：

| Role | 职责 | 工作流文档（lazy 加载） |
|---|---|---|
| **架构师** (architect) | 设计、评审、写决策文档 | `doc/ai/architecture/AGENTS.md` |
| **开发者** (developer) | 实现功能、调试、写代码、维护 LOG | `doc/ai/develop/AGENTS.md` |
| **审查者** (reviewer) | 代码评审、性能/安全/可访问性审计 | `doc/ai/review/AGENTS.md` |

### 每次会话开始的约定

**用户**：在 session 第一句话明确身份，例：「我是开发者，要做 …」。

**Codex**：根据声明，主动 read 对应的 `doc/ai/<role>/AGENTS.md`（lazy 加载触发），再开始工作。
