...
> **状态**：✅ 架构设计完成，可交付开发
>
> ⚠️ **命名更新（2026-06-15 arch3）**：本文 §5 的 "Session 1~6" 已废弃，统一映射为 `doc/ai/develop/TODO.md` 的 task6~task11，开发以 TODO 为准。映射：Session 1→task6（首页）、Session 2→task7（MC 后端）、Session 3→task8（MC 前端 + 工具页）、Session 4→task9（部署前整理）、Session 5→task10（Vercel + 域名）、Session 6→task11（部署后验证）。
